...
17.3 Happy git with R - create and connect a github repo with usethis
usethis::use_github()

make a change, save, commit, push - check out change on remote
...
:::{.callout-note}
Quarto Pub is a free publishing service for quarto content. It requires having a login and an access token. This is a relatively straight forward way to get your blog online. There are some limitations to the size and everything published on Quarto Pub is **publicly visible**. The link above outlines the steps to publish with Quart Pub.
:::
...
In the Site settings in the 'Publish directory' section at the bottom put the `_site/` folder where your website is rendered.

It will take a few mintues to deploy your website. Netlify automatically generates a random url like "lighthearted-travesseiro-492jfg3" which can be changed in the 'Domain settings'.
